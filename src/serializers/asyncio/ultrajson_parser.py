...
class StreamingJsonParser:
    ...
    def _fast_find_object_end(self, start_pos: int) -> int:
        """Fast scan for object end with minimal overhead."""
        brace_count = 0
        in_string = False
        escape_next = False
                    ...
                    if key_end > key_start:
                        ...
                        if colon_pos > key_end:
                            ...
                            if value_start < len(obj_str):
                                value = self._fast_extract_value(obj_str, value_start)
    ...
    def _fast_extract_value(self, obj_str: str, start_pos: int) -> Any:
    ...
    def _fast_extract_complete_pairs(self, obj: Dict[str, Any]) -> Dict[str, Any]:
        """Fast extraction of complete key-value pairs."""
        # Ultra-JSON style: minimal validation for speed
        complete_pairs = {}

        for key, value in obj.items():
            # Fast validation: just check key is non-empty string

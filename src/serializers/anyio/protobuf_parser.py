"""
Protocol Buffers streaming parser implementation with anyio async operations.

This module *previously* implemented an async streaming JSON parser with Protocol Buffers-style processing.
...
class StreamingJsonParser:
    ...
    def __init__(self):
        ...
        self._active_key: str | None = None # Stores the decoded string of the last fully parsed key
    ...
    def consume(self, chunk: str) -> None:
        ...
        if not isinstance(chunk, str):
            ...
            # For now, let's assume callers will adhere to `str` or we convert.
            return # Or raise TypeError("consume expects str")

        # Convert string chunk to bytes for internal bytearray buffer
...
# --- Original Async Protobuf-inspired helper classes (now unused by StreamingJsonParser) ---
@dataclass
class AsyncParserState: # Original class
    """Immutable state container for async Protocol Buffers parser."""
    buffer: str = ""
    parsed_data: Dict[str, Any] = field(default_factory=dict)

class AsyncProtobufValidator: # Original class
    """Async validator for Protocol Buffers-style documents."""
    @staticmethod
...
class AsyncProtobufExtractor: # Original class
    ...
    async def extract_complete_pairs(obj: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with anyio.create_task_group() as tg:
            for key, value in obj.items():
                tg.start_soon(AsyncProtobufExtractor._process_pair, key, value, result)
        return result
    @staticmethod
    async def _process_pair(key: str, value: Any, result: Dict[str, Any]) -> None:
        if await AsyncProtobufValidator.is_valid_key(key) and await AsyncProtobufValidator.is_valid_value(value):
            result[key] = value

class AsyncProtobufParser: # Original class
    """Async parser for individual Protocol Buffers-style documents."""
    def __init__(self, extractor: AsyncProtobufExtractor = None):
        self._extractor = extractor or AsyncProtobufExtractor()
...
class AsyncProtobufProcessor: # Original class
    """Main async processor using Protocol Buffers-inspired document processing."""
    def __init__(self, parser: AsyncProtobufParser = None):
        self._parser = parser or AsyncProtobufParser()
...
def get_metadata():
    """Returns metadata for the anyio Protobuf parser."""
    return {
        ...
        'best_use_case': 'High-performance async applications requiring Protocol Buffers support.'
    }
